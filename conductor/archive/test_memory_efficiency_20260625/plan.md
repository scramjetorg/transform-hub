...
- Guarded validation:
  ...
  - Guarded BDD bucket reruns passed: `npm run test:bdd-ci-node` (2 scenarios), `npm run test:bdd-ci-api-node` (20 scenarios), `npm run test:bdd-ci-hub` (14 lean scenarios), `npm run test:bdd-ci-verser2` (2 scenarios), `npm run test:bdd-appcontext` (7 scenarios / 40 steps after removing fixed waits), and `npm run test:bdd-ci-api-topic` (3 lean scenarios).
  - BDD fixes avoided timeout inflation: replaced a startup-config output assertion that waited for full stream end with bounded stream matching, added explicit hub cleanup to instance API scenarios, removed fixed AppContext waits where existing stream assertions already wait conditionally, replaced topic-delivery fixed waits with collected-data polling, and tagged long/redundant restart/topic scenarios as `@slow` for non-lean runs.
  - `npm run test:bdd-ci-python`: selected 0 scenarios under current tags and cleaned up without leaks; this remains a tag coverage issue rather than a memory blocker.
...
- Skipped validation: Docker-heavy `@requires-docker`/`@docker-specific`, `@stress`, `@load`, `@external-dependency`, aggregation-repro, and broad `test:bdd` suites are deferred because the lean guarded buckets now cover package runner behavior, host/API/topic/AppContext/verser2 smoke paths, and cleanup/leak detection without exercising intentionally heavy scenarios.
