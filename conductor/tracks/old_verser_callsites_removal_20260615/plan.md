...
- [ ] Task: Inventory old-verser and BPMux traces across active code, tests, config, package metadata, and docs
    - [ ] Search active source and tests for `@scramjet/verser`, `@scramjet/bpmux`, `VerserConnection`, `VerserClient`, `new Verser`, `apiVerser`, `verserConnection`, `BPMux`, `migrationMode`, `legacy`, and `dual`.
    - [ ] Classify each trace as active runtime callsite, active config switch, active compatibility test, package dependency, standalone `packages/verser`/`packages/bpmux` package code/test, transient invariant/doc proof, or historical/archive.
...
## Phase 3: Remove Transient Guardrails, Clean Old Tests, and Finalize Documentation

- [ ] Task: Final inventory before cleanup
    - [ ] Search the full repository for old-verser/BPMux identifiers, old config flags, active test names, docs, invariant allowlists, and package dependencies.
...
- [ ] Task: Clean active test suites and documentation
    ...
    - [ ] Update the verser2 rollout notes if this track supersedes remaining old-verser/BPMux cleanup items.
- [ ] Task: Final validation and commit Phase 3
    - [ ] Run repository-wide static searches proving no active old-verser/BPMux traces remain outside `packages/verser`, `packages/bpmux`, and historical/archive locations.
    ...
    - [ ] Run relevant BDD smoke validation for Manager/STH connectivity and record any skipped command with exact reason.
    - [ ] Commit the scoped Phase 3 changes.
- [ ] Task: Conductor - User Manual Verification 'Remove Transient Guardrails, Clean Old Tests, and Finalize Documentation' (Protocol in workflow.md)
