...
verser2 now provides in-process Host-side local peer attachment (`host.attachLocalBroker()`, `host.attachLocalGuest()`). For colocated Manager/STH use cases, the local peer path is the primary attachment: the Host owner registers Guest handlers and Broker request capability directly against the Host route/lease machinery without creating an extra HTTP/2 client connection. Local peers preserve the same authorization, duplicate peer ID checks, route table, lease lifecycle, streaming, and error semantics as networked H2 peers, with `local: true` metadata that cannot be spoofed by remote callers. Local peers also remove the need for loopback TLS/mTLS configuration for colocated components. Networked TLS H2 Guest/Broker remains the right path for remote participants, but remote participants must connect to their immediate upstream owner: STH connects to Manager/MultiManager, and runners/runtime wrappers connect only to their owning STH-local Host.
...
In platform-connected mode, STH may fetch or receive Manager/MultiManager public trust material for the `STH -> Manager/MultiManager` connection, and separately owns or receives authority for its STH-local runner/runtime Host. The Manager-local CA/server identity and STH-local runner CA/server identity are separate trust domains. A delegated runner/runtime CA must issue only runner/runtime identities and must not be accepted for Manager, MultiManager, or STH server identities. Role authorization must verify that the issuer is valid for the registering role.
...
Guest and Broker clients must use explicit trust material with inline `ca` PEM bundles or `caFile`. The preferred runner/STH-local handoff is the verser-team-supported inline `tls.ca` string containing one or more concatenated PEM certificates. In Node TLS, configuring `ca` replaces the default CA set for that connection. Deployments that need both public WebPKI and private platform CA trust must provide a combined CA bundle. `rejectUnauthorized: false` and `NODE_TLS_REJECT_UNAUTHORIZED=0` are prohibited outside isolated tests.

Runner and stack-runner trust bundles are generated by STH for the `Runner / Stack-Runner -> STH-local Host` connection. The bundle must always include the STH-local CA and should include the Manager CA when STH has fetched or configured it, using this order: STH-local CA first, Manager CA second. Including Manager public trust gives stack-runners the same trust context for STH-mediated flows without permitting direct runner-to-Manager transport. The runner `hostUrl` remains STH-local even when the runner trust bundle contains Manager CA material.
...
- CA bundle for the immediate upstream Host. Runner and stack-runner bundles are generated by STH and include STH-local CA plus Manager CA when available.
...
Private identity delivery uses files, not raw PEM/PFX values in environment variables or command arguments. Public CA trust may be delivered as inline `tls.ca` PEM bundles where supported, or as CA bundle files when a runtime/library/platform path requires files. Rotation writes new private material atomically, updates the allowlist, and reconnects peers. Keeping the same peer ID requires coordinated disconnect/reconnect because duplicate peer registration is fatal. Revocation removes allowlist entries and closes active connections; do not assume CRL/OCSP support unless the selected `verser2` package explicitly provides it. Cleanup removes key/cert files, Docker mount directories, Kubernetes Secrets, and generated CA bundle files when the sequence/instance exits or is deleted and no same-sequence local instance still references reusable material.
