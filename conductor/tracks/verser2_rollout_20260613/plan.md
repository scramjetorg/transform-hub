...
- [ ] Task: Migrate Node runtime
    ...
    - [ ] Add or update runner-node tests for non-listening `context.api` exposure by attaching a Node Guest route with `createVerserNodeGuest()`.
        - [~] Current stop point after compaction/halt: the active slice implements Host/Manager shared routed HTTP forwarding plus Host RPC forwarding over the existing runner route, rather than making `HostProxy` understand verser2 routes. `packages/api-server/src/handlers/routed-forward.ts` now provides `forwardRoutedRequest()` and `normalizeForwardedHeaders()`; `packages/manager/src/lib/manager.ts` is refactored to use the helper for Manager→STH routed forwarding; Host global `/api/v1/rpc` and direct instance `/rpc` paths now try `ICSI.forwardRpcRequest()` before legacy forwarding; `CSIController.forwardRpcRequest()` adapts the runner Broker route; synthetic verser2 CSI attach passes the real `hostProxy`; and the runner route server proxies non-channel requests to the runner-node API server once the monitoring PING exposes its host/port.
        - [~] URL-normalization behavior added in this slice: for v1 Host APIs, clients can call short RPC paths such as `/api/v1/rpc/<expose-path-tail>` without duplicating `/api/v1` inside the RPC path; explicit paths with different version numbers still require the full exposed path. Focused coverage added in `packages/host/test/rpc-url-normalization.spec.ts`.
        - [~] Validation already run for the current uncommitted slice: `npm exec -- ava test/rpc-url-normalization.spec.ts test/runner-transport.spec.ts` in `packages/host` passed; `npm exec -- ava test/transport/verser2-runner-transport.spec.ts` in `packages/runner` passed; `npm test -- test/rpc-url-normalization.spec.ts` and `npm run build` in `packages/host` passed; `npm run build` in `packages/api-server`, `packages/runner`, and `packages/manager` passed. One parallel `packages/manager` build failed with dist workspace install race/code 217 and passed when rerun serially, classified as transient/tooling.
        - [ ] Next resume step: add focused tests for shared routed forwarding and Host RPC broker forwarding/path stripping if feasible, request an Oracle review of the shared Host RPC routed-forwarding slice, then update this plan again before continuing the runtime migration.
