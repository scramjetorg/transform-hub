...
- [~] Task: Resolve all remaining classified BDD failures before scheduling
  ...
  Deferred runner/prerunner-image coverage: HUB-001 TC-009 through TC-012 remain tagged `@needs-fix` and are excluded from default BDD selection. They depend on repository-built Docker runner and prerunner images and short-lived auto-removed container metadata; a subsequent track must audit the runner-image and prerunner-image build workflows, repair them, and restore durable image/config assertions before re-enabling them. TC-012 (prerunner-image) shares the same root cause as TC-009 (runner-image): both reference container image tags from an internal registry (repo.int.scp.ovh/scramjet/…) and depend on pre-published image artifacts. There is no repository-built workflow that builds runner or prerunner images from the monorepo source and tags them for CI use. This is a scoped deferment, not a memory-guard skip.  To run the deferred scenarios during repair work, opt in with the env var and select the tag:
