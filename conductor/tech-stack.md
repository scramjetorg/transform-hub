...
- **Coverage**: no repository-owned coverage collector until the opt-in c8 mode is added to `scripts/run-ava.js`; metrics will cover remapped package TypeScript sources and exclude staged, build, dependency, generated, test, and coverage-output artifacts. No coverage threshold or CI gate is configured.
