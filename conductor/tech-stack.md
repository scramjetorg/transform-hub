...
- **Coverage**: opt-in c8 coverage runs through the supported AVA runner. Use `npm run test:packages -- --coverage` for the package-wide surface; `scripts/run-script.js` bridges this option through nested package test scripts without package-specific wrappers. Each tested package writes `coverage/lcov.info` and raw V8 data under `coverage/tmp`; metrics cover remapped `src/**/*.ts` and exclude staged, build, dependency, generated, test, and coverage-output artifacts. Metrics are newly established, with no historical-parity expectation, threshold, or CI gate.
...
- **Release alignment**: `npm run release:align:check -- --release-version=X.Y.Z`, `npm run release:align:dry-run -- --release-version=X.Y.Z`, and `npm run release:align:apply -- --release-version=X.Y.Z` are the supported package/dependency/image alignment commands. They share the fixed 2.0.0 inclusion boundary, preserve excluded packages, and gate release publication.
