...
## Design / Patterns
- **STH Connection Management**: Each connected STH is wrapped in an `STHController` instance; the `SthConnectionStore` holds the active map of controllers by ID. Supports transparent STH reconnection, id-based deduplication, and registration payload with `description`, `tags`, `accessKey`, and `routeDomain` fields. Re-registration replaces the active controller (with rollback on failure) and clears stale sequence/instance inventory in `STHInfoRegister`, re-populating from the new init cycle's emissions.
