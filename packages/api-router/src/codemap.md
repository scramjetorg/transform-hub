...
- **Adapter boundary**: `adapters/http.ts` registers routes on Cero-based `APIExpose` surfaces (including resolver middleware for verser2 redirect); `adapters/verser2.ts` registers routes on verser2 adapters.
...
- **Hook pipeline**: `hooks.ts` composes before/after/error/finally hooks (`executeRoutePipeline`) around route execution. Includes built-in `headerHook`, `corsHook`, `requestLoggingHook`.
...
- **Schema/OpenAPI**: `openapi.ts` generates OpenAPI 3.1 documents from manifests with Zod-to-JSON-Schema conversion. `schema-mode.ts` loads manifests from schema modules.
- **Validation**: `validation.ts` provides `validateRouteRequest()`/`validateRouteResponse()` against Zod schemas.
