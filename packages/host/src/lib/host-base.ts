...
import { APIRoute, AppConfig, IComponent, IObjectLogger, LogLevel, NextCallback, OpResponse, ParsedMessage, SequenceInfo, STHConfiguration, STHRestAPI } from "@scramjet/types";
 ...
 */
export class HostBase implements IComponent {
    /**
     * Configuration.
     */
    ...
    public get apiBase(): string {
        throw new Error("API base is not initialized.");
    }
    ...
    constructor(socketServer: SocketServer, sthConfig: STHConfiguration) {
        ...
        if (this.config.cpmUrl) {
            // Why CPMConnector and Verser client needs a server?
     ...
     */
    attachHostAPIs() {
        // Should use router or API server
        const api = this.api;
        ...
        api.downstream(`${apiBase}/sequence`,
            async (req) => this.handleNewSequence(req), { end: true }
        );

        api.op("delete", `${apiBase}/sequence/:id`, (req: ParsedMessage) => this.handleDeleteSequence(req));
        api.op("post", `${apiBase}/sequence/:id/start`, async (req: ParsedMessage) => this.handleStartSequence(req));

        api.get(`${apiBase}/sequence/:id`, (req) => this.getSequence(req.params?.id));
        api.get(`${apiBase}/sequence/:id/instances`, (req) => this.getSequenceInstances(req.params?.id));
        api.get(`${apiBase}/sequences`, () => this.getSequences());
        api.get(`${apiBase}/instances`, () => this.getInstances());

        api.get(`${apiBase}/load-check`, () => this.loadCheck.getLoadCheck());
        api.get(`${apiBase}/version`, () => ({ version }));

        api.get(`${apiBase}/topics`, () => this.serviceDiscovery.getTopics());
        api.downstream(`${apiBase}/topic/:name`, async (req) => {
        ...
        api.upstream(`${apiBase}/topic/:name`, (req: ParsedMessage, _res: ServerResponse) => {
            const params = req.params || {};
        ...
        api.upstream(`${apiBase}/log`, () => this.commonLogsPipe.getOut());

        api.use(`${this.instanceBase}/:id`, (req, res, next) => this.instanceMiddleware(req, res, next));
    }
