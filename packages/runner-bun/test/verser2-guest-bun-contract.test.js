...
if (!isolatedProcess) {
    test("runs the Verser Bun guest route contract in an isolated module cache", async () => {
        const child = Bun.spawn({
            cmd: [process.execPath, "test", import.meta.path],
            env: { ...process.env, SCRAMJET_RUNNER_BUN_CONTRACT_ISOLATED: "1" },
            stderr: "pipe",
            stdout: "pipe"
        });
        ...
        if (exitCode !== 0) {
            throw new Error(`Isolated Verser Bun guest route contract failed:\n${stdout}${stderr}`);
        }
    });
} else {
