...
| `lib/verser.ts` | `Verser` class (server-side) — listens for HTTP CONNECT, manages incoming VerserConnections, emits connect/close events. Extracts STH identity headers (`x-sth-tags`, `x-manager-id`, `x-sth-id`, `x-org-id`). |
