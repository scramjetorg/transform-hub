...
CLI entrypoint (`start.ts`) for the Multi-Manager process. Parses command-line options, assembles MultiManager configuration, creates the HTTP API server, and starts the `MultiManager` orchestrator.
...
6. `MultiManager` is instantiated with the server and config; `.start()` is called.
7. Optional heap-dump monitor dumps a V8 snapshot when memory exceeds a threshold.

## Data & Control Flow

`process.argv` → `parseCliOptions()` → `MultiManagerCommandOptions` → `MultiManagerConfig` (merged with file config and defaults) + `ServerConfiguration` (SSL) → `createServer()` → `new MultiManager(apiServer, mmConfig)` → `multiManager.start()`.
