...
- [~] Define **Release Finalizer** as a separate job in the protected `main` workflow, sourced from the main workflow revision and gated by a dedicated finalization environment. It verifies the main merge/tree relationship, candidate assets/admission/publication/verifier evidence and trusted GitHub Actions producer identities, then—and only then—uses narrowly scoped write permission to create the immutable `vX.Y.Z` tag and a draft GitHub Release, copy the verified candidate tarball assets into it without rebuilding, and download/hash-verify every copied asset against the candidate release-set SHA-256/SRI before publishing the release and attaching evidence. Phase 8 owns OCI image staging and mutable tag promotion.
...
- Remaining completion evidence is external: protected rulesets and admission policy, `production-publish` and `release-finalization` environments, candidate asset retention/access controls, and an approved staged release rehearsal.
- 2026-09-16 remote audit: existing devel/main rulesets do not enforce the exact admission/merge policy; `production-publish` and `release-finalization` environments do not exist; Actions defaults are write-scoped; no observable production-policy confirmation variable exists; candidate-release cleanup/access controls and per-package npm trusted publishers are unconfirmed; GHCR package permissions could not be inspected with the current token. The audit found two rulesets, so `remote-feasibility.json`'s zero-ruleset fact is stale, but its fail-closed conclusion remains correct.
