...
Feature: MANAGER-003 Full API capability through Verser2 forwarding
  ...
  @aggregation-repro-cleanup
  Scenario: MANAGER-003 TC-006 Manager restart preserves Verser2 forwarding and aggregation for connected hubs
    Given an isolated MultiManager aggregation stack
    And an STH hub "hub-1" is connected to the aggregation Manager
    And I wait for hubs to register with the Manager
    When I send a "GET" request through the aggregation Manager proxy to "/sth/hub-1/version" with headers "{}"
    Then the response status should be 200
    When the aggregation Manager is restarted
    And I wait for hubs to register with the Manager
  ...
  @aggregation-repro-cleanup
  Scenario: MANAGER-003 TC-007 Manager restart preserves Verser2 RPC forwarding for connected hubs
    Given an isolated MultiManager aggregation stack
    And an STH hub "hub-rpc" is connected to the aggregation Manager
    And I wait for hubs to register with the Manager
    When I eventually send a "POST" request through the aggregation Manager proxy to "/sth/hub-rpc/instance/hub-rpc-api-main/rpc/test/abc" with body "rpc-before-restart" and headers "{\"Content-Type\":\"text/plain\"}"
    Then the response status should be 200
    And the response body should be "POST /abc rpc-before-restart"
    When the aggregation Manager is restarted
    And I wait for hubs to register with the Manager
    When I query the Manager "/instances" through the MultiManager proxy
    Then the Manager proxy response should contain at least 1 items
