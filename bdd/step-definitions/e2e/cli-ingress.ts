...
When("two real CLI completion commands run against isolated session storage", async function(this: CustomWorld) {
    ...
    const artifact = resolveBddCliArtifact();
    symlinkSync(artifact.packageDir, join(completionRoot, "cli"), "dir");
    symlinkSync(artifact.nodeModulesDir, join(completionRoot, "node_modules"), "dir");
    symlinkSync(artifact.scriptsDir, join(completionRoot, "scripts"), "dir");
    const completionCli = ["node", "--preserve-symlinks", "--preserve-symlinks-main", join(completionRoot, "cli", artifact.binRelativePath)];
