...
When(
    ...
    async function (this: CustomWorld, internalPort: number, givenServer: string) {
        ...
        if (givenServer === "tcp") {
            ...
            console.log(
                `Attempting to connect on ${givenServer} port:${port}, host: ${host}`
            );
            ...
            this.resources.client = dgram.createSocket("udp4").on("error", (err) => {
...
When("check stream for message sent", async function (this: CustomWorld) {
    ...
    assert.notEqual(str.includes(this.resources.testMessage), null);
});
