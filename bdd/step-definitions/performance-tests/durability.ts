...
When(
    ...
    async function (this: CustomWorld, minNumber: number, seq: string) {
        ...
        do {
            ...
            // eslint-disable-next-line no-extra-parens
            if (
                (loadCheck as any).status !== 200 ||
                loadCheck.data?.memFree < 512 << 20
            ) {
                rejected = true;
            } else {
            ...
            if (rejected) {
                console.log(
                    "Sequence rejected. Total sequences started: ",
                    this.resources.instances.length
                );
            }
            ...
            await Promise.all(
                this.resources.instances.map(async (instance: InstanceClient) => {
                    ...
                    try {
                        ...
                        console.error(
                            `${instance.id}, sent: ${hash}, received: ${JSON.stringify(
                                response.data
                            )}`
                        );
                        rej(
